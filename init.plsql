...
CREATE TABLE EVENT_DUTIES_CONN (
    ...
    CONSTRAINT EVENT_DUTIES_CONN_U UNIQUE (EVENT_ID, DUTY_ID, MEMBER_ID),
    CONSTRAINT EVENT_DUTIES_CONN_EVENT_DUTY_U UNIQUE (EVENT_ID, DUTY_ID)
);
...
BEGIN
    ...
    INSERT INTO ADDRESS_HIS (
        ...
        V_OPERATOR,
        'Address ' || V_OPERATION || ': ' || :NEW.ZIP_CODE || ', ' || :NEW.COUNTRY || ', ' || :NEW.CITY || ', ' || :NEW.STREET_NAME || ', ' || :NEW.STREET_TYPE || ', ' || :NEW.HOUSE_NUMBER,
        SYSTIMESTAMP
    );
END;
/
...
BEGIN
    ...
    INSERT INTO RANK_PRIVILEGES_HIS (
        ...
        'Member ' || V_OPERATION || ': ' || :NEW.MEMBER_ID,
        SYSTIMESTAMP
    );
    COMMIT;
END;
